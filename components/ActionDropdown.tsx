...
const ActionDropdown = ({ file }: { file: Models.Document }) => {
  ...
  const handleAction = async () => {
    ...
    const actions = { // each are of different callback function
      rename: () =>renameFile({ fileId: file.$id, name, extension: file.extension, path }),
      share: () => updateFileUsers({ fileId: file.$id, emails, path }),
      delete: () =>deleteFile({ fileId: file.$id, bucketFileId: file.bucketFileId, path }),
      //bucket file id is passed because to delete a file from the original server also ...
    };
    success = await actions[action.value as keyof typeof actions]();// ensure this has only one of those three things
